...
# Modified slightly to allow colour changes without updates to the tree
...
class RGBXmasTree(SourceMixin, SPIDevice):
	...
	@brightness.setter
	def brightness_bits(self, brightnessInBits):
		if (brightnessInBits < 1):
			brightnessInBits = 1
		elif  (brightnessInBits > 31):
			brightnessInBits = 31
		self._brightness_bits = brightnessInBits
		self._brightness = brightnessInBits/31.0
		self.value = self.value
